...
def compare_text_file(
    ...
    **options,
    ...
    agent.logger.log(f"📄 Comparing text file for '{task.uid}'.", level=LogLevel.INFO)
    ...
    try:
        ...
        if not source_expected_file_path.is_file():
            agent.logger.log(f"❌ Expected file not found: '{source_expected_file_path}'", level=LogLevel.ERROR)
            return 0.0
        ...
        if actual_text == expected_text:
            agent.logger.log(f"✅ Text file comparison PASSED for '{task.uid}'. Files match.", level=LogLevel.INFO)
            return 1.0
        else:
            agent.logger.log(
                f"❌ Text file comparison FAILED for '{task.uid}'. Files do not match.", level=LogLevel.INFO
            )
            return 0.0

    except Exception as e:
        agent.logger.log(f"❌ Error during text file comparison for '{task.uid}': {e}", level=LogLevel.ERROR)
        return 0.0
