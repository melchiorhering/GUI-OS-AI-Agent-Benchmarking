...
from .constants import DEFAULT_POINTER_END_TOKEN, DEFAULT_POINTER_PAD_TOKEN, chat_template
...
def get_prediction_region_point(
    attn_scores, n_width, n_height, top_n=30, activation_threshold=0.3, return_all_regions=True, rect_center=False
):
    """
    ...
    for region in regions:
        ...
        if not rect_center:
            ...
            weighted_x = sum(nc[0] * w for nc, w in zip(normalized_centers, weights, strict=False)) / total_weight
            weighted_y = sum(nc[1] * w for nc, w in zip(normalized_centers, weights, strict=False)) / total_weight
...
def inference(conversation, model, tokenizer, data_processor, logits_processor=None, use_placeholder=False, topk=5):
    ...
    assiatant_starter = (
        ...
        else "<|im_start|>assistant<|recipient|>os\npyautogui.click(<|pointer_start|><|pointer_pad|><|pointer_end|>)"
    )
    ...
    # prepare text
    text = data_processor.apply_chat_template(
        conversation, tokenize=False, add_generation_prompt=False, chat_template=chat_template
    )
    text += assiatant_starter

    # prepare inputs
    image_inputs, video_inputs = process_vision_info(conversation)
    inputs = data_processor(text=[text], images=image_inputs, videos=video_inputs, padding=True, return_tensors="pt")
    ...
    best_point, region_points, region_scores, region_points_all = get_prediction_region_point(
        attn_scores, n_width, n_height, return_all_regions=True, rect_center=False
    )
