...
def run_single_example(
    agent: PromptAgent, env: DesktopEnv, example: dict, result_dir: str, args: argparse.Namespace
) -> float:
    ...
    while not done and step_idx < args.max_steps:
        ...
        for action in actions:
            ...
            if args.observation_space != "screenshot":
                with open(
                    os.path.join(a11y_tree, f"step_{step_idx + 1}_{action_timestamp}.txt"), "w", encoding="utf-8"
                ) as _f:
