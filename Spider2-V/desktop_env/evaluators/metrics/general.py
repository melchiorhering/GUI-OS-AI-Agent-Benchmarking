    ...
    for r in rules:
        ...
        if "text" in r:
            match_func: Callable[[str], Number] = functools.partial(
                operator.eq if r["exact"] else (lambda a, b: fuzz.ratio(a, b) / 100.0), r["text"]
            )
...
def compare_time_in_speedtest_results(speedtest_result_path, time_diff):
    ...
    try:
        with open(speedtest_result_path, "r") as f:
            ...
            if not abs(
                (
                    datetime.datetime.strptime(date_time, "%H:%M") - datetime.datetime.strptime(now_date_time, "%H:%M")
                ).total_seconds()
            ) / 60 < int(time_diff):
                return 0
        return 1
