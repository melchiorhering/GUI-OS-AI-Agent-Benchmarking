...
def config() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--path_to_vm",
        type=str,
        help="path to the VM executable .vmx file, if None, automatically find the VM in vm_data/ folder",
    )
    parser.add_argument(
        "-s",
        "--snapshot_name",
        type=str,
        default="init_state",
        help="Snapshot name to use (overwrite snapshot in each example config)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="Maximum number of steps for each example, this can be altered dynamically according to field `action_number` in the example config",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="maximum length of interaction history to provide to the agent",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="JSON dict containing example ids to run",
    )
    parser.add_argument("--exclude_account", action="store_true", help="Whether to use RAG for the agent")
    parser.add_argument(
        "--execution_feedback", action="store_true", help="whether to use execution feedback for the agent"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["all"],
        help="Application names list to filter examples",
    )
...
def test(args: argparse.Namespace, test_all_meta: List[dict]) -> dict:
    ...
    for example in tqdm(test_all_meta, desc="Example", leave=False):
        ...
        example_handler = logging.FileHandler(
            os.path.join(result_dir, "result-{:}.log".format(datetime_str)), encoding="utf-8"
        )
        ...
        try:
            ...
            with open(os.path.join(result_dir, "trajectory.jsonl"), "a") as f:
                f.write(json.dumps({"Error": f"Error msg in {domain}/{eid}: {e}"}))
                f.write("\n")
...
def get_examples(args, result_dir: str, easy_first: bool = True) -> List[Dict[str, str]]:
    ...
    for domain in os.listdir(data_dir):
        if domain not in filtered_domain or domain not in test_data:
            continue
        ...
        for example_id in test_data[domain]:
            ...
            if not args.from_scratch and os.path.exists(result_file) and file_not_empty(result_file):
                continue
            data_config = os.path.join(example_dir, f"{example_id}.json")
            data = json.load(open(data_config, "r"))
            if args.exclude_account and "account" in data["tags"]:
                continue
