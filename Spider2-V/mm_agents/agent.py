...
import openai
import tiktoken
from google.api_core.exceptions import BadRequest, InternalServerError, InvalidArgument, ResourceExhausted
from PIL import Image

from mm_agents.accessibility_tree_wrap.heuristic_retrieve import draw_bounding_boxes, filter_nodes
...
class PromptAgent:
    ...
    def get_current_cost(self) -> str:
        ...
        logger.info(
            f"[INFO]: Current usage: {self.usages['prompt_tokens'] * 1e-6:.2f}M prompt tokens, {self.usages['completion_tokens'] * 1e-6:.2f}M completion tokens, cost ${total_cost:.2f} ."
        )
        return

    def add_action_infos(
        self, messages, action_list: List[Union[str, Dict]], infos: List[Dict], failed_only: bool = True
    ) -> Dict:
        ...
        prefix_msg = (
            "Here are the parsed action list from your last response and the execution flag of each action from the desktop environment:\n"
            if not failed_only
            else "Here are the failed actions and their execution feedbacks of your last response:\n"
        )
        content_msg = ""
        ...
        if len(action_list) == 0:
            for action, info in zip(action_list, infos, strict=False):
        ...
        suffix_msg = (
            "\nIf there are any omissions or additions of actions, please meticulously check the specification of the action space; if the execution status of any action is incorrect or unexpected, try to resolve it based on the latest observations below."
            if not failed_only
            else "Please meticulously check the error messages and the execution status of the failed actions, and try to resolve them based on the latest observations below."
        )
    ...
    def predict(self, instruction: str, obs: Dict, context: str = None) -> List:
        ...
        for previous_obs, previous_action, previous_thought in zip(_observations, _actions, _thoughts, strict=False):
            # {{{1
                ...
                messages.append(
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "image_url",
                                "image_url": {"url": f"data:image/png;base64,{_screenshot}", "detail": "high"},
                            },
                        ],
                    }
                )
        ...
        if self.observation_space in ["screenshot", "screenshot_a11y_tree"]:
            ...
            if self.observation_space == "screenshot_a11y_tree":
                ...
                self.observations.append(
                    {"screenshot": base64_image, "raw_screenshot": obs["screenshot"], "accessibility_tree": None}
                )
    ...
    @backoff.on_exception(
        ...
        (
            # OpenAI exceptions
            openai.RateLimitError,  # Reverted to native OpenAI exceptions
            openai.APITimeoutError,  # Added API Timeout error
            openai.APIConnectionError,  # Added API Connection error
            openai.APIStatusError,  # General API status error for non-200 responses
            # Google exceptions (keep as they were, they are not specific to OpenAI client)
            InvalidArgument,
            ResourceExhausted,
            InternalServerError,
            BadRequest,
    ...
    def call_llm(self, payload):
        ...
        try:
            response = self.client.chat.completions.create(  # Use client instance
                model=self.model,
        ...
        except openai.APIStatusError as e:  # Catch specific OpenAI API status errors
            if e.response.status_code == 400 and "context_length_exceeded" in e.response.text:
                logger.error("Context length exceeded. Retrying with a smaller context.")
                payload["messages"] = [payload["messages"][0]] + payload["messages"][-1:]
                try:
                    retry_response = self.client.chat.completions.create(  # Use client instance for retry
                        model=self.model,
                        messages=payload["messages"],
                        max_completion_tokens=self.max_tokens,
                        # temperature=self.temperature,
                    ...
                    logger.error(f"Failed to call LLM even after attempt on shortening the history: {retry_e}")
                    return ""
            else:
